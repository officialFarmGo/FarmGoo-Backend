...
exports.signUpValidator = (req, res, next) =>{
    const schema = joi.object({
        firstName: joi.string().trim().pattern(/^[A-Za-z]+$/).required().messages({
               ...
               'string.pattern.base': "First name cannot contain numbers, spaces, or special characters",
               }),
               lastName: joi.string().trim().pattern(/^[A-Za-z]+$/).required().messages({
                   'any.required': "Last Name is required",
                   "string.empty": "Last name cannot be empty",
                   'string.pattern.base': "Last name cannot contain numbers, spaces, or special characters",
               }), 
               ...
               townOrVillage: joi.string().trim().pattern(/^[A-Za-z\s]+$/).required().messages({
                   'any.required': "Town or Village is required",
                   "string.empty": "Town or Village cannot be empty",
                   'string.pattern.base': "Town or Village cannot contain numbers, spaces, or special characters",
               }), 
